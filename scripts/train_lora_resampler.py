...
DEFAULT_NEIGHBORS = 4
DEFAULT_RESAMPLER_LEARNING_RATE = 5e-4
...
def prepare_example(
    ...
    reduced_tokens = resampler(vision_tokens, source_grid, target_grid)
...
def main(
    ...
    resampler = LocalResampler(
        hidden_size=base_model.config.vision_config.hidden_size,
        neighbors=neighbors,
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {
                "params": list(resampler.parameters()),
                "lr": resampler_learning_rate,
            },
        ]
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loss < best_loss:
            ...
            resampler.save_pretrained(run_dir / "best")

    trained_model.save_pretrained(run_dir / "last")
    resampler.save_pretrained(run_dir / "last")
    record = {
        "model": MODEL_NAME,
        ...
        "reduction_method": "local-resampler",
        "neighbors": neighbors,
        "epochs": epochs,
        "learning_rate": learning_rate,
        "resampler_learning_rate": resampler_learning_rate,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Fine-tune Gemma 4 E4B LoRA with a learned local resampler."
    )
    ...
    parser.add_argument(
        "--resampler-learning-rate",
        type=float,
        default=DEFAULT_RESAMPLER_LEARNING_RATE,
    )
