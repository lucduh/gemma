...
def run_inference(
    ...
    inputs = prepare_inputs(processor, image, prompt, source_image_tokens)
    target_grid = target_soft_grid(
        image, processor.image_processor, target_image_tokens
    )
    target_visual_tokens = target_grid[0] * target_grid[1]
    inputs = compact_image_placeholders(
        inputs, model.config.image_token_id, target_visual_tokens
    )
    ...
    start = time.perf_counter()
    image_features = project_vision(model, pooled_tokens)
    synchronize(device)
    ...
    visual_token_info = {
        ...
        "source_tokens": source_grid[0] * source_grid[1],
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate Gemma 4 E4B with post-encoder visual-token pooling."
    )
